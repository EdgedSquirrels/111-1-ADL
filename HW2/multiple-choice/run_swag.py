...
class DataTrainingArguments:
    ...
    context_file: Optional[str] = field(
        default=None,
        metadata={"help": "An optional context data file of the paragraphs."},
    )
    test_file: Optional[str] = field(
        default=None,
        metadata={"help": "An optional test data that will make prediction to predict file."},
    )
    predict_file: Optional[str] = field(
        default=None,
        metadata={"help": "An optional output file that contains the predictions."},
    )
...
def main():
    ...
    if data_args.train_file is not None or data_args.validation_file is not None or data_args.test_file is not None:
        ...
        # print("context:", data_args.context_file)
    ...
    def preprocess_function(examples):
        ...
        # context_name = "sent1"
        # question_header_name = "sent2"
        # first_sentences = [[context_data[id]] * 4 for id in examples['relevant']]
        first_sentences = [[question] * 4 for question in examples['question']]
        second_sentences = [[context_data[id] for id in ids] for ids in examples['paragraphs']]
        ...
        # Un-flatten
        batch = {k: [v[i : i + 4] for i in range(0, len(v), 4)] for k, v in tokenized_examples.items()}
        # Add label

        if 'relevant' in examples.keys():
            batch['label'] = [ids.index(id) for ids, id in zip(examples['paragraphs'], examples['relevant'])]
        # print(batch.keys()) # input_ids, token_type_ids, attention_mask, label
        return batch
    ...
    if training_args.do_predict:
        ...
        with open(data_args.predict_file, 'w', encoding='utf-8') as f:
